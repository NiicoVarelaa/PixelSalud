...
const crearMensaje = (req, res) => {
  ...
  if (!idCliente || !nombre || !email || !mensaje) {
    return res
      .status(400)
      .json({
        error: "Faltan datos obligatorios (idCliente, nombre, email, mensaje).",
      });
  }
  if (
  ...
  conection.query(insertQuery, params, async (err, result) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ error: "Error interno al guardar el mensaje." });
    }
    // Enviar email de confirmación (no bloquear respuesta si falla)
    enviarConfirmacionCliente(email, nombre, asunto || "Sin Asunto").catch(
      (e) => console.error("Error enviando email de confirmación:", e),
    );
    res
      .status(201)
      .json({
...
const listarMensajes = (req, res) => {
  conection.query(
    "SELECT * FROM MensajesClientes ORDER BY fechaEnvio DESC",
    (err, results) => {
      if (err) {
        return res
          .status(500)
          .json({ error: "Error al obtener los mensajes." });
      }
      res.json(results);
    },
  );

...
class CarritoService {
   ...
   */
  async agregarProducto(idCliente, idProducto, cantidad = 1) {
    // Validar cantidad
   ...
   * @returns {Promise<Object>} Resultado de la operación
   */
  async incrementarCantidad(idCliente, idProducto) {
    // Verificar que el item existe en el carrito
    const itemExiste = await carritoRepository.existsItem(
      idCliente,
      idProducto,
    );
    if (!itemExiste) {
   ...
   * @returns {Promise<Object>} Resultado de la operación
   */
  async decrementarCantidad(idCliente, idProducto) {
    // Verificar que el item existe en el carrito
    const itemExiste = await carritoRepository.existsItem(
      idCliente,
      idProducto,
   ...
   * @returns {Promise<Object>} Resultado de la operación
   */
  async eliminarProducto(idCliente, idProducto) {
   ...
   */
  async obtenerEstadisticas(idCliente) {
    const items = await carritoRepository.findByClienteWithProducts(idCliente);

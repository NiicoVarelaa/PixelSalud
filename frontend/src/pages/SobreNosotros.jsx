        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 divide-x divide-slate-100">
          {STATS_DATA.map(({ id, number, label, Icon }) => (
            ...
            >
              ...
              <span className="text-xs md:text-sm text-slate-500 font-semibold uppercase tracking-wider">
                {label}
              </span>
            </div>
          ))}
      ...
      <div className="max-w-6xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="space-y-8">
            <div>
              <h2 className="text-3xl md:text-4xl font-bold text-slate-900 mb-6 relative inline-block">
                Nuestra Filosofía
                <span
                  className="absolute -bottom-2 left-0 w-1/3 h-1.5 bg-primary-500 rounded-full"
                  aria-hidden="true"
                ></span>
              </h2>
            </div>
            <p className="text-lg text-slate-600 leading-relaxed">
              ...
              bienestar. No solo dispensamos medicamentos; construimos
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              {["Atención Farmacéutica", "Dermocosmética", "Nutrición"].map(
                (tag) => (
                  <div
                    key={tag}
                    className="flex items-center gap-2 px-4 py-2 bg-slate-50 border border-slate-200 rounded-full text-slate-700 font-medium text-sm hover:bg-slate-100 transition-colors"
                  >
          ...
          <div className="grid grid-cols-2 gap-4 relative">
            <div
              className="absolute -inset-4 bg-primary-100/50 rounded-full blur-3xl -z-10 opacity-50"
              aria-hidden="true"
            />
            <img
              src={ampollas}
  ...
  return (
    <section className="py-20">
      <div className="max-w-5xl mx-auto bg-linear-to-r from-primary-700 to-primary-900 rounded-3xl p-12 text-center relative overflow-hidden shadow-2xl">
        <div
          className="absolute inset-0 opacity-10 bg-[radial-gradient(#ffffff_1px,transparent_1px)] bg-size-[16px_16px]"
          aria-hidden="true"
        ></div>

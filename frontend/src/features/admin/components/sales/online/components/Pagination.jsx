...
export const Pagination = () => {
  ...
  const { totalPaginas, totalItems } = useMemo(() => {
    const ventasFiltradas = ventas.filter((v) => {
      ...
      const estadoVenta = String(v.estado || "").toLowerCase();
      const estadoFiltro = String(filtroEstado || "todas").toLowerCase();
  ...
  return (
    <div className="w-full bg-white border border-gray-200/80 rounded-2xl px-4 py-4 shadow-sm">
      <nav
        ...
        role="navigation"
        aria-label="Paginacion de ventas online"
      >

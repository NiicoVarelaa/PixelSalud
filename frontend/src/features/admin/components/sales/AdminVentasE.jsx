...
const AdminVentasE = () => {
  ...
  const itemsActuales = ventasFiltradas.slice(
    indiceUltimoItem - itemsPorPagina,
    indiceUltimoItem,
  );
                ...
                <tbody className="divide-y divide-gray-100">
                  {itemsActuales.length > 0 ? (
                    itemsActuales.map((venta) => (
                      <tr
                        key={venta.idVentaE}
                        className={`hover:bg-gray-50 transition-colors ${venta.estado === "anulada" ? "bg-red-50/50" : ""}`}
                      >
                        ...
                        <td className="px-2 py-3 text-center">
                          <span
                            className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${venta.estado === "completada" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                          >
                            {venta.estado}
                          </span>
                        </td>
                          ...
                          <div className="flex gap-1 justify-center">
                            {venta.estado === "completada" ? (
                              ...
                              !!permisos.modificar_ventasE && (
                                <button
                                  onClick={() =>
                                    handleReactivar(venta.idVentaE)
                                  }

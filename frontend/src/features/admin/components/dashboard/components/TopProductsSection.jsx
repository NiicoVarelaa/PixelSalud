...
const TopProductsSection = ({ productos, loading }) => {
  ...
  const prevSlide = () => {
    if (canScroll) {
      setCurrentIndex((prev) => 
        prev - itemsPerPage < 0 ? Math.max(0, totalProductos - itemsPerPage) : prev - itemsPerPage
      );
    }
  };

  const visibleProducts = productos.slice(currentIndex, currentIndex + itemsPerPage);
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-2 px-1">
        <h2 id="top-products-heading" className="text-sm sm:text-base font-extrabold text-gray-900">
      ...
      <div className="flex-1 relative overflow-hidden">
        {loading ? (
          <div className="grid grid-cols-3 gap-2 h-full">
            {[...Array(3)].map((_, index) => (
              <div key={index} className="bg-gray-50 border border-gray-100 rounded-xl h-full min-h-40 animate-pulse flex flex-col overflow-hidden">
          ...
          <div className="flex flex-col items-center justify-center h-full min-h-40 text-center bg-gray-50/50 rounded-xl border-2 border-dashed border-gray-200 p-2">
            ...
            <p className="text-gray-600 font-semibold text-xs">Sin ventas registradas</p>
          </div>
        ) : (
          <div className="grid grid-cols-3 gap-2 h-full">
